...
def main(context):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, ema_model, optimizer, epoch, training_log, torch_device)
...
def train(train_loader, model, ema_model, optimizer, epoch, log, torch_device):
    global global_step
    ...
    for i, ((input, ema_input), target) in enumerate(train_loader):
        ...
        assert not (np.isnan(float(loss)) or float(loss) > 1e5), 'Loss explosion: {}'.format(float(loss))
...
def accuracy(output, target, topk=(1,)):
    ...
    maxk = max(topk)
    labeled_minibatch_size = max(float((target != NO_LABEL).sum()), 1e-8)
